...
const SupplierScreen = () => {
  ...
  const handleClearAll = async () => {
    try {
      const response = await fetch("https://supplierdata.runasp.net/api/deletesuppliers", {
        method: "DELETE"
      });
      if (response.ok) {
        setSuppliers([]);
  ...
  const proceedImport = async (clearExisting) => {
    ...
    try {
      if (clearExisting) {
        const clearResp = await fetch("https://supplierdata.runasp.net/api/deletesuppliers", {
          method: "DELETE"
        });
        if (!clearResp.ok) {
      ...
      const response = await fetch("https://supplierdata.runasp.net/api/importcsv", {
        method: "POST",
        body: formData,
      });
